...
class SqlGenerator:
    ...
    def helper_top_down(
        ...
        current_sql_table_name: Final[str] = f'{self.config.xml_table_name}{self.alias_num:03}'

        if self.treenodeinfo.is_branch():
            ...
            if tni.is_leaf() is True:
                sql_data_type = tni.data_type.value
                ...
                for child in tni.children:
                    if child.is_array is True:
                        ...
                        sql_data_type = child.data_type.value
                        if sql_data_type != DataTypes.STR.value:
                        ...
                        self.sql_select.append(f'    "{current_sql_table_name}"."{child.xpath}",')
                        self.sql_from.append(f'               "{child.xpath}" {sql_data_type} PATH \'{child.element_name}\',')
    ...
    def top_down_sql_generation(self) -> None:
        self.alias_num = 1
        current_sql_table: Final[str] = f'{self.config.xml_table_name}{self.alias_num:03}'
        current_table_number: Final[int] = self.alias_num
        ...
        self.sql_select.append(f'        "{self.config.dv_datasource_name}".getFiles(\'{self.config.dv_datasource_filepath}\') f')
